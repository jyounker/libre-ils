...
//-----------------------------------------------------------------------------
//
// $Id: php-marc.php,v 1.1 2005/04/08 08:56:29 lanttis Exp $
// $Revision: 1.1 $
//
//-----------------------------------------------------------------------------
	...
	function decode($text) {
		...
		for ($n=0; $n<$nfields; $n++) {
			...
			if ( preg_match("/^\d+$/", $tagno) && ($tagno < 10) ) {
				...
				$subfield_data = array();
				$i=0;  //added by JTY to squash 'x' subfields getting overwritten
				foreach ($subfields as $subfield) {
					if ( strlen($subfield) > 0 ) {
						if(substr($subfield,0,1) == "x") { //added by JTY
							$x2 = "x".$i; //added by JTY
							$subfield_data[$x2] = substr($subfield, 1);$i++; //added by JTY
						} else { //added by JTY
